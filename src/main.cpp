...
#include <glad/glad.h> // SIEMPRE antes que GLFW
#include <glad/glad.h> // SIEMPRE antes que GLFW
...
<<<<<<< HEAD
    ...
    float teslaFlicker = (sin(currentFrame * 40.0f) * 0.5f + 0.5f) *
                         (cos(currentFrame * 10.0f) * 0.5f + 0.5f);
    if (teslaFlicker < 0.2f)
      teslaFlicker = 0.0f; // Silencio electrico
    ...
    if (currentLightIdx < 12) {
      ...
      glm::vec3 teslaLightPos =
          teslaPos +
          glm::vec3(0.0f, 1.2f, 0.0f); // Elevar la luz al tope de la bobina
      glUniform3fv(pointLightPosLoc[currentLightIdx], 1,
                   glm::value_ptr(teslaLightPos));
      // Azul cian electrico
      glUniform3f(pointLightColLoc[currentLightIdx], 0.2f * teslaFlicker,
                  0.5f * teslaFlicker, 1.0f * teslaFlicker);
...
>>>>>>> 8f5c484 (la cague, hice pull en main y se me olvido cambiarme a mi rama)
...
<<<<<<< HEAD
        ...
        unsigned int pIzquiVAO = 0, pIzquiVBO = 0, pDereVAO = 0, pDereVBO = 0;
        int pIzquiCount = 0, pDereCount = 0;
        loadOBJMesh("assets/puertaizqui.obj", pIzquiVAO, pIzquiVBO,
                    pIzquiCount);
        loadOBJMesh("assets/puertadere.obj", pDereVAO, pDereVBO, pDereCount);
        ...
        GLTFModel *mirrorGLTF = new GLTFModel("assets/mirror.glb");
        GLTFModel *mirrorBGGLTF = new GLTFModel("assets/mirrorBG.glb");
        GLTFModel *ligthbathroom2GLTF =
            new GLTFModel("assets/ligthbathroom.glb");
        GLTFModel *ligthbathroomGLTF =
            new GLTFModel("assets/ligthbathroom.glb");
        mensBGLTF = new GLTFModel("assets/mensB.glb");
        girlBGLTF = new GLTFModel("assets/girlB.glb");
        banoGLTF = new GLTFModel("assets/Bano.glb");
        ...
        reactorControlGLTF =
            new GLTFModel("assets/contencion/reactor-control.glb");
        warningGLTF = new GLTFModel("assets/contencion/warning.glb");
        std::cout << "[SISTEMA] Props baño cargados: "
                  << "Lampara(" << ligthbathroomGLTF->meshes.size() << "), "
                  << "Bano(" << banoGLTF->meshes.size() << "), "
                  << "Lavamanos(" << lavamanosGLTF->meshes.size() << "), "
                  << "Urinario(" << urinarioGLTF->meshes.size() << ")"
                  << std::endl;
        std::cout << "[SISTEMA] Props contención cargados: "
                  << "Tesla(" << teslaGLTF->meshes.size() << "), "
                  << "Esquineros(" << esquinerosGLTF->meshes.size() << ")"
        ...
        unsigned int doorTex =
            loadTextureWithFallback("assets/puerta_metal.png", 0);
        ...
        int outerCutOffLoc = glGetUniformLocation(shaderProgram, "outerCutOff");
        int flashlightOnLoc =
            glGetUniformLocation(shaderProgram, "flashlightOn");

        int dimAlternaLoc =
            glGetUniformLocation(shaderProgram, "dimensionAlterna");
        int zoneLoc = glGetUniformLocation(shaderProgram, "currentZone");
        int timeLoc = glGetUniformLocation(shaderProgram, "time");
        int resLoc = glGetUniformLocation(shaderProgram, "resolution");
        int solidColorLoc =
        ...
        int numPointLightsLoc =
            glGetUniformLocation(shaderProgram, "numPointLights");
        int pointLightPosLoc[12];
        int pointLightColLoc[12];
        int pointLightRadLoc[12];
        for (int i = 0; i < 12; i++) {
          ...
          radStr += ".radius";
          pointLightPosLoc[i] =
              glGetUniformLocation(shaderProgram, posStr.c_str());
          pointLightColLoc[i] =
              glGetUniformLocation(shaderProgram, colStr.c_str());
          pointLightRadLoc[i] =
              glGetUniformLocation(shaderProgram, radStr.c_str());
        }
        ...
        // paredes, piso y techo. Las coords son celdas del grid (igual que el
        // worldMap). Tip: camina al cuarto en el juego y lee cameraPos.x /
        // cameraPos.z en el editor para encontrar las coordenadas exactas del
        // área que quieres cubrir.
        //
        unsigned int wallTex4 = 0;
        unsigned int wallTex5 = 0;
        ...
        while (!glfwWindowShouldClose(window)) {
          ...
          glUseProgram(shaderProgram);
          glVertexAttrib3f(3, 1.0f, 1.0f,
                           1.0f); // Default obj color para otros VAOs
          glUniform1f(emissiveStrengthLoc,
                      0.0f); // Por defecto nada emite luz propia
...
>>>>>>> 8f5c484 (la cague, hice pull en main y se me olvido cambiarme a mi rama)
          ...
          for (int z = 0; z < MAP_HEIGHT; ++z) {
            for (int x = 0; x < MAP_WIDTH; ++x) {
              ...
              debugModel = glm::translate(
                  debugModel,
                  glm::vec3((float)x, (wallHeight - 1.0f) * 0.5f, (float)z));
              debugModel = glm::scale(debugModel, glm::vec3(scaleX + 0.02f,
                                                            wallHeight + 0.02f,
                                                            scaleZ + 0.02f));
              glUniformMatrix4fv(modelLoc, 1, GL_FALSE,
      ...
      if (showDebugGUI) {
        ...
        ImGui::DragFloat3("ReactorCtrl Rot", &reactorControlRot.x, 0.5f,
                          -180.0f, 180.0f);
        ImGui::DragFloat3("ReactorCtrl Scale", &reactorControlScale.x, 0.01f,
                          0.01f, 10.0f);
        ...
        ImGui::Text("Lampara Reactor");
        ImGui::DragFloat3("LamparaR Pos", &lamparaReactorPos.x, 0.05f);
        ImGui::DragFloat3("LamparaR Rot", &lamparaReactorRot.x, 0.5f, -180.0f,
                          180.0f);
        ImGui::DragFloat3("LamparaR Scale", &lamparaReactorScale.x, 0.01f,
                          0.01f, 10.0f);
        if (ImGui::Button("Traer Lampara Reactor frente a camara")) {
          lamparaReactorPos = cameraPos + cameraFront * 2.0f;
          lamparaReactorPos.y = -0.5f;
        }
        ImGui::Separator();
        ...
        ImGui::SetNextWindowPos(ImVec2((float)currentWidth - 350.0f, 360.0f),
                                ImGuiCond_Always);
        ImGui::SetNextWindowSize(ImVec2(340.0f, 330.0f), ImGuiCond_Always);
        ImGui::SetNextWindowBgAlpha(0.78f);
        ImGui::Begin("Editor Entidades", NULL,
                     ImGuiWindowFlags_NoResize | ImGuiWindowFlags_NoMove);
        ...
        if (gameEntities.empty()) {
          ...
          if (ImGui::BeginListBox("Entidades", ImVec2(0.0f, 120.0f))) {
            for (int i = 0; i < (int)gameEntities.size(); ++i) {
              const Entity &entity = gameEntities[i];
              if (entityOnlyCollectibles &&
                  !(entity.type == 0 || entity.type == 1 || entity.type == 8 ||
                    entity.type == 9))
                continue;

              std::string label =
                  std::to_string(i) + " - " + getEntityTypeLabel(entity.type);
          ...
          ImGui::DragFloat("Seed", &selectedEntity.seed, 0.05f, -100.0f,
                           100.0f);
          ...
          ImGui::InputTextMultiline("##entity_export", exportLine.data(),
                                    exportLine.size() + 1, ImVec2(0.0f, 70.0f),
        ...
        if (showInteractionDebugger) {
          ...
          int focusedIndex =
              findFocusedEntityIndex(&focusedDistance, &focusedAngle);
          int doorGridX = 0;
          int doorGridZ = 0;
          int doorBlockType = 0;
          bool hasDoorAhead =
              findDoorAhead(doorGridX, doorGridZ, doorBlockType);
          ...
          ImGui::Begin("Interaction Debugger", NULL,
                       ImGuiWindowFlags_NoResize | ImGuiWindowFlags_NoMove);
          ImGui::Text("Camara dir: %.2f %.2f %.2f", cameraFront.x,
          ...
          if (focusedIndex >= 0) {
            ...
            ImGui::TextColored(ImVec4(1.0f, 0.7f, 0.3f, 1.0f),
                               "No hay entidad enfocada.");
          }

          ImGui::Separator();
          if (hasDoorAhead) {
            bool hasAccess = (doorBlockType == 8 || doorBlockType == -8)
                                 ? hasKeycardLvl1
                                 : hasKeycardLvl2;
            ImGui::Text("Puerta enfrente: %s",
                        getDoorDebugLabel(doorBlockType));
        ...
        if (showAnimationTester) {
          ...
          ImGui::SetNextWindowBgAlpha(0.78f);
          ImGui::Begin("Animation Tester", NULL,
                       ImGuiWindowFlags_NoResize | ImGuiWindowFlags_NoMove);
          if (!gnomeGLTF) {
            ImGui::TextColored(ImVec4(1.0f, 0.4f, 0.4f, 1.0f),
                               "No se cargo gnome.glb");
          } else {
            int animCount = gnomeGLTF->GetAnimationCount();
            ...
            ImGui::SliderFloat("Velocidad", &gnomeAnimSpeed, 0.0f, 3.0f,
                               "%.2f");
            ...
            if (animCount > 0) {
              ...
              ImGui::SliderInt("Indice anim", &gnomeDebugAnimIndex, 0,
                               animCount - 1);
              std::string animName =
                  gnomeGLTF->GetAnimationName(gnomeDebugAnimIndex);
              float animLength =
                  gnomeGLTF->GetAnimationLengthSeconds(gnomeDebugAnimIndex);
              if (animName.empty())
                animName = "(sin nombre)";
              ...
              if (gnomeAnimPreviewTime > maxPreviewTime)
                gnomeAnimPreviewTime = maxPreviewTime;
              ImGui::SliderFloat("Pose / tiempo", &gnomeAnimPreviewTime, 0.0f,
                                 maxPreviewTime, "%.2f s");
...
<<<<<<< HEAD
          ...
          // color de la lampara y posicion para ponerla en un color amarillento
          // poner colores mas altos en vez de 0.8 0.9 1.0 a unos 0.8 0.6 0.2
          // para que se vea mas amarillento
          glUniform3fv(pointLightPosLoc[0], 1,
                       glm::value_ptr(ligthbathroomPos));
          glUniform3f(pointLightColLoc[0], 0.8f * flicker1, 0.6f * flicker1,
                      0.2f * flicker1);
          glUniform1f(pointLightRadLoc[0], 4.0f);

          // Lámpara 2
          glUniform3fv(pointLightPosLoc[1], 1,
                       glm::value_ptr(ligthbathroom2Pos));

          == == == =
>>>>>>> 8f5c484 (la cague, hice pull en main y se me olvido cambiarme a mi rama)
                       auto findEntityByType = [&](int type) -> int {
            for (int i = 0; i < (int)gameEntities.size(); ++i) {
              if (gameEntities[i].type == type && gameEntities[i].active)
                return i;
            }
            return -1;
          };
...
<<<<<<< HEAD
          ...
          // lampara contencion 1
          glUniform3fv(pointLightPosLoc[4], 1,
                       glm::value_ptr(lamparaContencionPos));
          glUniform3f(pointLightColLoc[4], 0.8f * lampFlicker,
                      0.9f * lampFlicker, 1.0f * lampFlicker);
          glUniform1f(pointLightRadLoc[4], 4.0f);

          // lampara contencion 2 (tenue)
          glUniform3fv(pointLightPosLoc[5], 1, glm::value_ptr(lampara2Pos));
          glUniform3f(pointLightColLoc[5], 0.4f * lampFlicker,
                      0.45f * lampFlicker, 0.5f * lampFlicker);
          glUniform1f(pointLightRadLoc[5], 3.0f);

          // lampara contencion 3 (tenue)
          glUniform3fv(pointLightPosLoc[6], 1, glm::value_ptr(lampara3Pos));
          glUniform3f(pointLightColLoc[6], 0.4f * lampFlicker,
                      0.45f * lampFlicker, 0.5f * lampFlicker);
          glUniform1f(pointLightRadLoc[6], 3.0f);
          ...
          for (size_t i = 0; i < emergencyPos.size() && currentLightIdx < 11;
            ...
            glUniform3fv(pointLightPosLoc[currentLightIdx], 1,
                         glm::value_ptr(emergencyPos[i]));
            glUniform3f(pointLightColLoc[currentLightIdx],
                        1.0f * emergencyPulse, 0.0f, 0.0f);
            glUniform1f(pointLightRadLoc[currentLightIdx],
                        2.0f); // Radio corto para evitar traspasar paredes
            currentLightIdx++;
          }
          ...
          if (currentLightIdx < 12) {
            ...
            glm::vec3 teslaLightPos =
                teslaPos +
                glm::vec3(0.0f, 1.2f,
                          0.0f); // Elevar la luz al tope de la bobina
            glUniform3fv(pointLightPosLoc[currentLightIdx], 1,
                         glm::value_ptr(teslaLightPos));
            // Azul cian electrico
            glUniform3f(pointLightColLoc[currentLightIdx], 0.2f * teslaFlicker,
          ...
          for (int z = 0; z < MAP_HEIGHT; z++) {
            for (int x = 0; x < MAP_WIDTH; x++) {
              ...
              if (renderBlock != 0 &&
                ...
                // Detectar si esta celda es la primera o segunda de un par de
                // puertas
                bool isSecondDoorCell = false;
                if (is3DDoor && x > 0) {
                  int prevBlock = worldMap[z][x - 1];
                ...
                if (is3DDoor && isSecondDoorCell) {
                  ...
                  // 4. Mover al centro del hueco y anclar al piso de la pared
                  // (-0.5)
                  baseModel = glm::translate(
                      baseModel, glm::vec3((float)x + 0.5f, -0.5f, (float)z));
                  ...
                  // Alto Blender: 1.535 -> Juego: 1.0 (Escala 0.651)
                  baseModel =
                      glm::scale(baseModel, glm::vec3(1.1f, 0.651f, 1.1f));

                  // 2. Rotar (dejaremos 0 grados asumiendo que los nuevos estan
                  // de frente) Si se ven las texturas por detras, cambiaremos
                  // este valor a 180 despues. baseModel =
                  // glm::rotate(baseModel, glm::radians(180.0f),
                  // glm::vec3(0.0f, 1.0f, 0.0f));

                  // 1. Compensar el offset original de Blender para centrar el
                  // ensamble en (0,0,0) Centro X = 0.455, Centro Z = 0.059
                  baseModel = glm::translate(baseModel,
                                             glm::vec3(-0.455f, 0.0f, -0.059f));
                  ...
                  if (doorTex > 0) {
                    ...
                    glUniform1i(solidColorLoc, 1);
                    if (renderBlock == 8 || renderBlock == -8) {
                      glUniform3f(colorLoc, 1.0f, 0.8f,
                                  0.2f); // Amarillo SÃ³lido
                    } else if (renderBlock == 9 || renderBlock == -9) {
                      glUniform3f(colorLoc, 0.9f, 0.1f, 0.1f); // Rojo SÃ³lido
                    } else {
                      glUniform3f(colorLoc, 1.0f, 1.0f, 1.0f);
                  ...
                  // Obtener Ã¡ngulo actual de la animaciÃ³n
                  float currentAnim = (renderBlock == 8 || renderBlock == -8)
                                          ? door1Anim
                                          : door2Anim;

                  // Rotacion: La izquierda gira hacia adelante (negativo), la
                  // derecha gira hacia el otro lado (positivo)
                  float angleL = glm::radians(-currentAnim);
                  float angleR = glm::radians(currentAnim);
                  ...
                  if (pIzquiCount > 0) {
                    ...
                    glm::vec3 hingeL(-0.45f, 0.0f, 0.057f);
                    modelL = glm::translate(modelL, hingeL);
                    modelL = glm::rotate(modelL, angleL,
                                         glm::vec3(0.0f, 1.0f, 0.0f));
                    modelL = glm::translate(modelL, -hingeL);

                    glUniformMatrix4fv(modelLoc, 1, GL_FALSE,
                                       glm::value_ptr(modelL));
                    glDrawArrays(GL_TRIANGLES, 0, pIzquiCount);
                  }
                  if (pDereCount > 0) {
                    glBindVertexArray(pDereVAO);
                    ...
                    glm::vec3 hingeR(1.359f, 0.0f, 0.061f);
                    modelR = glm::translate(modelR, hingeR);
                    modelR = glm::rotate(modelR, angleR,
                                         glm::vec3(0.0f, 1.0f, 0.0f));
                    modelR = glm::translate(modelR, -hingeR);

                    glUniformMatrix4fv(modelLoc, 1, GL_FALSE,
                                       glm::value_ptr(modelR));
                    glDrawArrays(GL_TRIANGLES, 0, pDereCount);
                  }
                  ...
                  glm::mat4 model = glm::mat4(1.0f);
                  model = glm::translate(
                      model, glm::vec3((float)x, (wallHeight - 1.0f) * 0.5f,
                                       (float)z));
                  model =
                      glm::scale(model, glm::vec3(scaleX, wallHeight, scaleZ));
                  glUniformMatrix4fv(modelLoc, 1, GL_FALSE,
                                     glm::value_ptr(model));
                  glUniform3f(colorLoc, 1.0f, 1.0f, 1.0f);
                  glDrawArrays(GL_TRIANGLES, 0, 36);
              ...
              { // Techo en TODAS las celdas (incluidas las de pared, para
                // cubrir huecos de paredes delgadas)
                glm::mat4 roofModel = glm::mat4(1.0f);
                roofModel = glm::translate(
                    roofModel, glm::vec3((float)x, wallHeight, (float)z));
                glUniformMatrix4fv(modelLoc, 1, GL_FALSE,
                                   glm::value_ptr(roofModel));
                if (isContencionCell) {
                  glBindTexture(GL_TEXTURE_2D, roofContencionTex);
                  glUniform3f(colorLoc, 1.0f, 1.0f, 1.0f);
                } else {
          ...
          if (gnomeGLTF) {
            ...
            if (gnomeTexture == 0 && !gnomeTextureFailed) {
              ...
              if (gnomeTexture == 0) {
                gnomeTextureFailed = true;
                std::cout
                    << "[SISTEMA] No se encontró assets/Gnome_Albedo.png. El "
                       "gnomo usará sus colores por defecto."
                    << std::endl;
              }
            }
            ...
            if (isGnomeActive) {
              ...
              if (isFlashlightOn) {
                float angle = glm::dot(cameraFront, dirToGnome);
                // 0.98 significa que lo miras casi al centro (un cono muy
                // cerrado)
                if (angle > 0.98f && distToPlayer < 10.0f) {
                  beingLookedAt = true;
                }
              ...
              if (beingLookedAt) {
                ...
                gnomeStunTimer =
                    (std::max)(0.0f,
                               gnomeStunTimer - deltaTime); // El timer baja si
                                                            // dejas de mirarlo
              }

              // 3. MOVIMIENTO (Solo si NO lo está mirando o no ha sido
              // aturdido)
              float speed = 1.8f;
              bool isMoving = false;
              if (!beingLookedAt && distToPlayer > 0.8f) {
              ...
              // levantarse en espacio global)--
              float angle =
                  atan2(cameraPos.x - gnomePos.x, cameraPos.z - gnomePos.z);
              gnomeModel =
                  glm::rotate(gnomeModel, angle, glm::vec3(0.0f, 1.0f, 0.0f));
              ...
              if (!hasSkinningBones) {
                ...
                // GLB actual: escala de depuración más visible y sin giro extra
                gnomeModel =
                    glm::scale(gnomeModel, glm::vec3(0.07f, 0.07f, 0.07f));
              }
              ...
              // Actualizar y enviar matrices de huesos para skinning (solo si
              // el modelo realmente trae huesos)
              if (hasSkinningBones) {
                gnomeGLTF->UpdateAnimation(gnomeRenderTime, gnomeBoneTransforms,
                                           currentAnimIndex);
                if (finalBonesLoc >= 0 && !gnomeBoneTransforms.empty()) {
                  glUniformMatrix4fv(
                      finalBonesLoc, (GLsizei)gnomeBoneTransforms.size(),
                      GL_FALSE, glm::value_ptr(gnomeBoneTransforms[0]));
                }
              }
              if (isAnimatedLoc >= 0) {
                glUniform1i(isAnimatedLoc, hasSkinningBones ? 1 : 0);
          ...
          // --- DECORACIÓN BAÑO (GLB estáticos) ---
          // Limpiar estado incondicionalmente antes de dibujar los props para
          // evitar heredar colores o estados
          glActiveTexture(GL_TEXTURE0);
          if (isAnimatedLoc >= 0)
            glUniform1i(isAnimatedLoc, 0);
          glUniform1i(solidColorLoc, 0);
          glUniform3f(colorLoc, 1.0f, 1.0f,
                      1.0f); // MUY IMPORTANTE: Resetear color que pudo dejar el
                             // techo oscuro del mapa
          glBindVertexArray(VAO);
          ...
          if (lavamanosGLTF && !lavamanosGLTF->meshes.empty()) {
            ...
            lavamanosModel = glm::translate(lavamanosModel, lavamanosPos);
            lavamanosModel =
                glm::rotate(lavamanosModel, glm::radians(lavamanosRot.x),
                            glm::vec3(1.0f, 0.0f, 0.0f));
            lavamanosModel =
                glm::rotate(lavamanosModel, glm::radians(lavamanosRot.y),
                            glm::vec3(0.0f, 1.0f, 0.0f));
            lavamanosModel =
                glm::rotate(lavamanosModel, glm::radians(lavamanosRot.z),
                            glm::vec3(0.0f, 0.0f, 1.0f));
            lavamanosModel = glm::scale(lavamanosModel, lavamanosScale);
            glUniformMatrix4fv(modelLoc, 1, GL_FALSE,
                               glm::value_ptr(lavamanosModel));
            lavamanosGLTF->Draw(shaderProgram, solidColorLoc);
          ...
          if (ligthbathroomGLTF && !ligthbathroomGLTF->meshes.empty()) {
            glm::mat4 ligthbathroomModel = glm::mat4(1.0f);
            ligthbathroomModel =
                glm::translate(ligthbathroomModel, ligthbathroomPos);
            ligthbathroomModel = glm::rotate(ligthbathroomModel,
                                             glm::radians(ligthbathroomRot.x),
                                             glm::vec3(1.0f, 0.0f, 0.0f));
            ligthbathroomModel = glm::rotate(ligthbathroomModel,
                                             glm::radians(ligthbathroomRot.y),
                                             glm::vec3(0.0f, 1.0f, 0.0f));
            ligthbathroomModel = glm::rotate(ligthbathroomModel,
                                             glm::radians(ligthbathroomRot.z),
                                             glm::vec3(0.0f, 0.0f, 1.0f));
            ligthbathroomModel =
                glm::scale(ligthbathroomModel, ligthbathroomScale);

            // Compensar el offset original del modelo en Blender para centrarlo
            // en su pivote real
            ligthbathroomModel = glm::translate(
                ligthbathroomModel, glm::vec3(-0.423f, -2.7725f, 2.622f));
          ...
          if (ligthbathroom2GLTF && !ligthbathroom2GLTF->meshes.empty()) {
            ...
            ligthbathroom2Model =
                glm::translate(ligthbathroom2Model, ligthbathroom2Pos);
            ligthbathroom2Model = glm::rotate(ligthbathroom2Model,
                                              glm::radians(ligthbathroom2Rot.x),
                                              glm::vec3(1.0f, 0.0f, 0.0f));
            ligthbathroom2Model = glm::rotate(ligthbathroom2Model,
                                              glm::radians(ligthbathroom2Rot.y),
                                              glm::vec3(0.0f, 1.0f, 0.0f));
            ligthbathroom2Model = glm::rotate(ligthbathroom2Model,
                                              glm::radians(ligthbathroom2Rot.z),
                                              glm::vec3(0.0f, 0.0f, 1.0f));
            ligthbathroom2Model =
                glm::scale(ligthbathroom2Model, ligthbathroom2Scale);
          ...
          if (mirrorBGGLTF && !mirrorBGGLTF->meshes.empty()) {
            ...
            mirrorBGModel = glm::translate(mirrorBGModel, mirrorBGpos);
            mirrorBGModel =
                glm::rotate(mirrorBGModel, glm::radians(mirrorBGRot.x),
                            glm::vec3(1.0f, 0.0f, 0.0f));
            mirrorBGModel =
                glm::rotate(mirrorBGModel, glm::radians(mirrorBGRot.y),
                            glm::vec3(0.0f, 1.0f, 0.0f));
            mirrorBGModel =
                glm::rotate(mirrorBGModel, glm::radians(mirrorBGRot.z),
                            glm::vec3(0.0f, 0.0f, 1.0f));
            mirrorBGModel = glm::scale(mirrorBGModel, mirrorBGScale);
            glUniformMatrix4fv(modelLoc, 1, GL_FALSE,
                               glm::value_ptr(mirrorBGModel));
            mirrorBGGLTF->Draw(shaderProgram, solidColorLoc);
          }
          ...
          if (urinarioGLTF && !urinarioGLTF->meshes.empty()) {
            ...
            urinarioModel = glm::translate(urinarioModel, urinarioPos);
            urinarioModel =
                glm::rotate(urinarioModel, glm::radians(urinarioRot.x),
                            glm::vec3(1.0f, 0.0f, 0.0f));
            urinarioModel =
                glm::rotate(urinarioModel, glm::radians(urinarioRot.y),
                            glm::vec3(0.0f, 1.0f, 0.0f));
            urinarioModel =
                glm::rotate(urinarioModel, glm::radians(urinarioRot.z),
                            glm::vec3(0.0f, 0.0f, 1.0f));
            urinarioModel = glm::scale(urinarioModel, urinarioScale);
            glUniformMatrix4fv(modelLoc, 1, GL_FALSE,
                               glm::value_ptr(urinarioModel));
            urinarioGLTF->Draw(shaderProgram, solidColorLoc);
          }
          ...
          if (lamparaContencionGLTF && !lamparaContencionGLTF->meshes.empty()) {
            ...
            lampModel = glm::translate(lampModel, lamparaContencionPos);
            lampModel =
                glm::rotate(lampModel, glm::radians(lamparaContencionRot.x),
                            glm::vec3(1.0f, 0.0f, 0.0f));
            lampModel =
                glm::rotate(lampModel, glm::radians(lamparaContencionRot.y),
                            glm::vec3(0.0f, 1.0f, 0.0f));
            lampModel =
                glm::rotate(lampModel, glm::radians(lamparaContencionRot.z),
                            glm::vec3(0.0f, 0.0f, 1.0f));
            lampModel = glm::scale(lampModel, lamparaContencionScale);
          ...
          if (emergencyGLTF && !emergencyGLTF->meshes.empty()) {
            for (size_t i = 0; i < emergencyPos.size(); i++) {
              ...
              emergModel = glm::translate(emergModel, emergencyPos[i]);
              emergModel =
                  glm::rotate(emergModel, glm::radians(emergencyRot[i].x),
                              glm::vec3(1.0f, 0.0f, 0.0f));
              emergModel =
                  glm::rotate(emergModel, glm::radians(emergencyRot[i].y),
                              glm::vec3(0.0f, 1.0f, 0.0f));
              emergModel =
                  glm::rotate(emergModel, glm::radians(emergencyRot[i].z),
                              glm::vec3(0.0f, 0.0f, 1.0f));
              emergModel = glm::scale(emergModel, emergencyScale[i]);

              glUniformMatrix4fv(modelLoc, 1, GL_FALSE,
                                 glm::value_ptr(emergModel));
              // Usar el pulso compartido y aumentar intensidad emisiva
              glUniform1f(emissiveStrengthLoc, 4.0f * emergencyPulse);
              emergencyGLTF->Draw(shaderProgram, solidColorLoc);
              glUniform1f(emissiveStrengthLoc, 0.0f);
          ...
          if (machineLabGLTF && !machineLabGLTF->meshes.empty()) {
            for (int i = 0; i < 3; i++) { // Dibujamos las 3 instancias
              ...
              machineLabModel =
                  glm::translate(machineLabModel, machineLabPos[i]);
              machineLabModel =
                  glm::rotate(machineLabModel, glm::radians(machineLabRot[i].x),
                              glm::vec3(1.0f, 0.0f, 0.0f));
              machineLabModel =
                  glm::rotate(machineLabModel, glm::radians(machineLabRot[i].y),
                              glm::vec3(0.0f, 1.0f, 0.0f));
              machineLabModel =
                  glm::rotate(machineLabModel, glm::radians(machineLabRot[i].z),
                              glm::vec3(0.0f, 0.0f, 1.0f));
              machineLabModel = glm::scale(machineLabModel, machineLabScale[i]);
              glUniformMatrix4fv(modelLoc, 1, GL_FALSE,
                                 glm::value_ptr(machineLabModel));
          ...
          if (metalDeskGLTF && !metalDeskGLTF->meshes.empty()) {
            for (int i = 0; i < 8; i++) { // Dibujamos las 8 instancias
              ...
              metalDeskModel =
                  glm::rotate(metalDeskModel, glm::radians(metalDeskRot[i].x),
                              glm::vec3(1.0f, 0.0f, 0.0f));
              metalDeskModel =
                  glm::rotate(metalDeskModel, glm::radians(metalDeskRot[i].y),
                              glm::vec3(0.0f, 1.0f, 0.0f));
              metalDeskModel =
                  glm::rotate(metalDeskModel, glm::radians(metalDeskRot[i].z),
                              glm::vec3(0.0f, 0.0f, 1.0f));
              metalDeskModel = glm::scale(metalDeskModel, metalDeskScale[i]);
              glUniformMatrix4fv(modelLoc, 1, GL_FALSE,
                                 glm::value_ptr(metalDeskModel));
          ...
          if (panelControlGLTF && !panelControlGLTF->meshes.empty()) {
            ...
            panelModel = glm::translate(panelModel, panelControlPos);
            panelModel =
                glm::rotate(panelModel, glm::radians(panelControlRot.x),
                            glm::vec3(1.0f, 0.0f, 0.0f));
            panelModel =
                glm::rotate(panelModel, glm::radians(panelControlRot.y),
                            glm::vec3(0.0f, 1.0f, 0.0f));
            panelModel =
                glm::rotate(panelModel, glm::radians(panelControlRot.z),
                            glm::vec3(0.0f, 0.0f, 1.0f));
            panelModel = glm::scale(panelModel, panelControlScale);
            glUniformMatrix4fv(modelLoc, 1, GL_FALSE,
                               glm::value_ptr(panelModel));
            panelControlGLTF->Draw(shaderProgram, solidColorLoc);
          }
