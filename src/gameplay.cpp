...
bool checkCollision(float x, float z) {
  ...
  if (mensBGLTF && !mensBGLTF->meshes.empty()) {
    ...
    model = glm::rotate(model, glm::radians(mensBrot.x),
                        glm::vec3(1.0f, 0.0f, 0.0f));
    model = glm::rotate(model, glm::radians(mensBrot.y),
                        glm::vec3(0.0f, 1.0f, 0.0f));
    model = glm::rotate(model, glm::radians(mensBrot.z),
                        glm::vec3(0.0f, 0.0f, 1.0f));
  ...
  if (girlBGLTF && !girlBGLTF->meshes.empty()) {
    ...
    model = glm::rotate(model, glm::radians(girlBrot.x),
                        glm::vec3(1.0f, 0.0f, 0.0f));
    model = glm::rotate(model, glm::radians(girlBrot.y),
                        glm::vec3(0.0f, 1.0f, 0.0f));
    model = glm::rotate(model, glm::radians(girlBrot.z),
                        glm::vec3(0.0f, 0.0f, 1.0f));
  ...
  if (urinarioGLTF && !urinarioGLTF->meshes.empty()) {
    ...
    model = glm::rotate(model, glm::radians(urinarioRot.x),
                        glm::vec3(1.0f, 0.0f, 0.0f));
    model = glm::rotate(model, glm::radians(urinarioRot.y),
                        glm::vec3(0.0f, 1.0f, 0.0f));
    model = glm::rotate(model, glm::radians(urinarioRot.z),
                        glm::vec3(0.0f, 0.0f, 1.0f));
  ...
  if (mirrorGLTF && !mirrorGLTF->meshes.empty()) {
    ...
    glm::vec3 scales[4] = {mirrorScale, mirrorScale2, mirrorScale3,
                           mirrorScale4};
    for (int i = 0; i < 4; i++) {
      glm::mat4 model = glm::mat4(1.0f);
      model = glm::translate(model, positions[i]);
  ...
  if (mirrorBGGLTF && !mirrorBGGLTF->meshes.empty()) {
    ...
    model = glm::rotate(model, glm::radians(mirrorBGRot.x),
                        glm::vec3(1.0f, 0.0f, 0.0f));
    model = glm::rotate(model, glm::radians(mirrorBGRot.y),
                        glm::vec3(0.0f, 1.0f, 0.0f));
    model = glm::rotate(model, glm::radians(mirrorBGRot.z),
                        glm::vec3(0.0f, 0.0f, 1.0f));
  ...
  for (const auto &prop : placedProps) {
    ...
    if (prop.modelName == "consola") {
      ...
      modelMat = glm::rotate(modelMat, glm::radians(prop.rot.x),
                             glm::vec3(1.0f, 0.0f, 0.0f));
      modelMat = glm::rotate(modelMat, glm::radians(prop.rot.y),
                             glm::vec3(0.0f, 1.0f, 0.0f));
      modelMat = glm::rotate(modelMat, glm::radians(prop.rot.z),
                             glm::vec3(0.0f, 0.0f, 1.0f));
  ...
  if (generadorGLTF && !generadorGLTF->meshes.empty()) {
    for (int i = 0; i < 3; ++i) {
      ...
      model = glm::rotate(model, glm::radians(generadorRot[i].x),
                          glm::vec3(1.0f, 0.0f, 0.0f));
      model = glm::rotate(model, glm::radians(generadorRot[i].y),
                          glm::vec3(0.0f, 1.0f, 0.0f));
      model = glm::rotate(model, glm::radians(generadorRot[i].z),
                          glm::vec3(0.0f, 0.0f, 1.0f));
      model = glm::scale(model, generadorScale[i]);
      if (checkModelCollision(generadorGLTF, model, playerPos, playerRadius))
        return true;
    }
  }
...
void updateZone() {
  if (cameraPos.z >= 35.0f)
    currentZone = 1;
  else if (cameraPos.z >= 15.0f)
    currentZone = 2;
  else
    currentZone = 3;
}
...
void processInput(GLFWwindow *window) {
  ...
  if (glfwGetKey(window, GLFW_KEY_G) == GLFW_PRESS) {
    if (!gKeyWasPressed) {
      ...
      gKeyWasPressed = true;
      ma_engine_play_sound(&audioEngine, "assets/click.wav", NULL);
      std::cout << "[GUI DEPURACION]: " << (showDebugGUI ? "VISIBLE" : "OCULTO")
                << std::endl;
    }
  } else {
  ...
  if (glfwGetKey(window, GLFW_KEY_H) == GLFW_PRESS) {
    if (!hKeyWasPressed) {
      ...
      hKeyWasPressed = true;
      ma_engine_play_sound(&audioEngine, "assets/click.wav", NULL);
      std::cout << "[SISTEMA]: Visor de Hitboxes "
                << (showCollisionViewer ? "ACTIVADO" : "DESACTIVADO")
                << std::endl;
    }
  } else {
  ...
  if (glfwGetKey(window, GLFW_KEY_LEFT_SHIFT) == GLFW_PRESS && stamina > 0.0f &&
      !isExhausted) {
    cameraSpeed = 6.0f;
  ...
  if (glm::length(moveDir) > 0.0f) {
    ...
    int moveSteps =
        (glm::max)(1, static_cast<int>(ceil(glm::length(moveDir) / 0.08f)));
  ...
  if (glfwGetKey(window, GLFW_KEY_E) == GLFW_PRESS) {
    if (!eKeyWasPressed) {
      ...
      if (!portalActivado && currentZone == 3) {
        float distA_Consola =
            glm::length(cameraPos - glm::vec3(25.0f, 0.0f, 7.0f));
  ...
  for (auto &entity : gameEntities) {
    ...
    if (entity.type == 0 || entity.type == 1 || entity.type == 8 ||
      ...
      if (entityLookAngle < 0.5f) {
        ...
        if (!checkCollision(entity.pos.x + realDirToEntity.x * speed,
                            entity.pos.z))
          entity.pos.x += realDirToEntity.x * speed;
        if (!checkCollision(entity.pos.x,
                            entity.pos.z + realDirToEntity.z * speed))
          entity.pos.z += realDirToEntity.z * speed;
        entity.pos.y = 0.0f;
      }
...
void mouse_callback(GLFWwindow *window, double xposIn, double yposIn) {
  if (gameState != PLAYING || !isCursorLocked)
    return;
...
void printNodeHierarchy(const aiNode *node, int depth) {
  ...
  std::cout << "- Node: " << node->mName.C_Str() << " | Pos: (" << position.x
